...
__help__ = """
❍ `` / warn <userhandle> `*: * احصل على رقم المستخدم وسبب التحذيرات. 
❍ `/ warnlist` *: * قائمة بجميع عوامل تصفية التحذير الحالية * المسؤولون فقط: *
❍` / warn <userhandle> `*: * تحذير المستخدم. بعد 3 تحذيرات ، سيتم حظر المستخدم من المجموعة. يمكن أيضا أن تستخدم كرد. 
❍ `` / dwarn <userhandle> `*: * تحذير المستخدم وحذف الرسالة. بعد 3 تحذيرات ، سيتم حظر المستخدم من المجموعة. يمكن أيضا أن تستخدم كرد. 
❍ `` / resetwarn <userhandle> `*: * إعادة تعيين التحذيرات للمستخدم. يمكن أيضا أن تستخدم كرد.
❍ `/ addwarn <كلمة رئيسية> <رسالة الرد>` *: * اضبط عامل تصفية تحذير على كلمة رئيسية معينة. إذا كنت تريد أن تكون كلمتك الرئيسية \ جملة ، فقم بتضمينها بعلامات اقتباس ، مثل: `/ addwarn" very angry "هذا مستخدم غاضب". 
❍ `/ nowarn <keyword>` *: * إيقاف عامل تصفية التحذير 
❍ '/ warnlimit <num> `*: * تعيين حد التحذير 
❍` / strongwarn <on / yes / off / no> `*: * إذا تم التعيين على تشغيل ، سيؤدي تجاوز حد التحذير إلى حظر. عدا ذلك ، سوف يضرب فقط.
"""
